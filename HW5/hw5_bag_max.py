...
def f1_score(y_true,y_pred):# {{{
    ...
    return ta_f1_score(y_true, y_prob)
# }}}
...
checkpoint = ModelCheckpoint(filepath='best_weights_{}.h5'.format(ID), verbose=1, save_best_only=True, save_weights_only=True, monitor='val_f1_score', mode='max')
...
model.load_weights('./best_weights_{}.h5'.format(ID))
...
with open(output_path,'w') as output:
    ...
    y_prob_max = np.max(Y_pred, axis=1).reshape(Y_pred.shape[0], 1)
    y_thres = y_prob_max * thresh
    Y_pred_thresh = (Y_pred > y_thres).astype('int')
