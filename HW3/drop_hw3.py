...
def load_vali_data(): #with execute# {{{
    # load from file
    vali_data = pandas.read_csv(VALI_FILE, encoding='big5')
    vali_data = vali_data.values

    # generate y_vali_data# {{{
    y_vali_data = vali_data[:,0].reshape(vali_data.shape[0], 1)
    y_vali = np_utils.to_categorical(y_vali_data, 7)# }}}

    # generate x_vali_data# {{{
    # split vali
    x_vali_data = np.zeros((vali_data.shape[0], 2304), int)
    for i in range(vali_data.shape[0]):
        x_vali_data[i,:] = np.fromstring(vali_data[i,1], dtype=np.int, sep=' ')
    x_vali_data = x_vali_data.astype(float)
    # normalize
    x_vali_data = normalize(x_vali_data)
    # }}}

    return (x_vali_data, y_vali)
(x_vali_data, y_vali) = load_vali_data()# }}}
...
model = load_model('./tmp_300.hdf5')
model.fit(x_train, y_train, batch_size=BATCH_SIZE, epochs=EPOCHS, validation_data=(x_vali, y_vali))
model.save('tmp_350.hdf5')
model.fit(x_train, y_train, batch_size=BATCH_SIZE, epochs=EPOCHS, validation_data=(x_vali, y_vali))
model.save('tmp_400.hdf5')
